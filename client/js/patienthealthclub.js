...
client.httpGetAsync(client.SERVER_URL + "patient/healthclub/" + entityID, function(response){printPatient(response);});

document.getElementById("nav-healthclub").onmousedown = function(){
...
let decryptButton = function(id){
  ...
  document.getElementById('addNotesDiv'+id).innerHTML = '<label>Add Training-related Health Data</label>'
...
function addNote(id){
  ...
  var policy = "patient = " + userInfo[id].patientID + " or healthclub = " + entityID;
  let final_enc = crypto.encryptPolicy(policy, dec_notes);
  let encryptedEntity = {};
  // Name encryption
  encryptedEntity.data = final_enc;

  client.httpPutAsync(client.SERVER_URL + "patient/" + userInfo[id].patientID + "/healthclub/" + entityID, encryptedEntity, function(response){
    console.log(response);
    window.location.replace("patienthealthclub.html?type=" + entityType + "&id=" + entityID);
  });
}
  ...
  if (userInfo.length > 0) {
    for (let i = 0; i < userInfo.length; i++) {
      ...
      divPatient.innerHTML = '<dl class="dl-horizontal">'
        ...
        +  '<dt>Training-Related Data</dt><dd id="patientData' + i + '">' + (userInfo[i]['data'] ? userInfo[i]['data']['data'] : '') + '</dd>'
        + '</dl>'
        + '<div id="addNotesDiv' + i + '"></div>'
        + '<input id="addNote' + i + '" type="button" style="display: none;" value="Add Training-related Data" onmousedown="addNote('+i+')">'
        + '<div id="messages"></div>'
    ...
    alert("No patients are associated to health club ID="+entityID);
  }
}
